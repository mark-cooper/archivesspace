...
#      @rep_image = get_rep_image(@result['json']['instances'])
  ...
  def digitized
    ...
    # TODO: refactor, see lifted from inventory
    if !@results.blank?
      params[:q] = '*'
  ...
  # TODO: refactor, lifted from fetch_containers
  def fetch_digital_objects(resource_uri, page_uri, params)
    qry = "collection_uri_u_sstr:\"#{resource_uri}\" AND (#{DIGITAL_QUERY})"
    @base_search = "#{page_uri}?"
    search_opts =  default_search_opts({
      'sort' => 'title_sort asc',
      'facet.mincount' => 1
    ...
    search_opts['fq']=[qry]
    set_up_search(['pui_digital_object'], ['digital_object_type_enum_s', 'extent_type_enum_s', 'subjects', 'published_agents', 'langcode'], search_opts, params, qry)
    @base_search= @base_search.sub("q=#{qry}", '')
    page = Integer(params.fetch(:page, "1"))
